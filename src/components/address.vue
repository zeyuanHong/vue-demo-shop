<template>
    <van-nav-bar title="收货地址" left-arrow @click-left="hideAddress('cart')" fixed placeholder />
    <van-address-list v-model="chosenAddressId" default-tag-text="默认" :list="address" @add="onAdd" @edit="onEdit"
        @select="onSelect" />
    <van-popup v-model:show="show">
        <van-address-edit :address-info="initInfo" :area-list="areaList" show-postal :show-delete="isDelete"
            show-set-default :search-result="searchResult" :area-columns-placeholder="['请选择', '请选择', '请选择']"
...
export default {
    ...
    mounted() {
        ...
        for (let i = 0; i < this.address.length; i++) {
            if (this.address[i].isDefault) { // 找到默认地址并传递到父组件
    ...
    methods: {
        ...
        onSave(content) { // 点击保存地址，content是地址数据
            ...
            // 更新全局状态和后端数据库的地址数据。
            this.setProfileAddressAction({ data: this.address })
                .then(() => {
