<template>
    <van-nav-bar fixed placeholder="true" :title="getTitle" left-arrow right-text="全部" @click-left="onClickLeft" @click-right="onClickRight" />
    <van-search  shape="round" v-model="value" @search="handleSearch" placeholder="请输入搜索关键词" />
...
export default {
    ...
    watch: {
        $route(route) { // 监听路由变化，route是路由对象
            if (route.name === "list") { // 判断路由是在list页面才执行操作
