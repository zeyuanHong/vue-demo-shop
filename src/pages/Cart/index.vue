  ...
  <div v-show="showCom === 'cart'">
    ...
    >
      ...
      >
        <dt>
          <van-checkbox
            :name="item.id + '-' + item.color + '-' + item.size"
          ></van-checkbox>
        </dt>
        <dd>
          <!-- 商品卡片 -->
          ...
          >
            <template #tags>
              <van-tag v-if="item.color" plain type="danger"
                >颜色:{{ item.color }}</van-tag
              >
              <van-tag v-if="item.size" plain type="danger"
                >尺寸{{ item.size }}</van-tag
              >
            </template>
            <template #footer>
    ...
    <van-submit-bar
      ...
      @submit="onSubmit"
    >
      <van-checkbox @click="handleCheckAll" v-model="checkAll"
        >全选</van-checkbox
      >
      <van-button
        @click="handleDel"
      ...
      <template #tip>
        {{ selectAddressShow }}
        <van-button size="mini" type="default" @click="showCom = 'address'"
          >选择地址</van-button
        >
      </template>
    </van-submit-bar>
...
export default {
  ...
  computed: {
    ...
    totalPrice() {
      ...
      this.cartData.forEach((item) => {
        // console.log(this)
        if (
          this.checked.includes(item.id + "-" + item.color + "-" + item.size)
        ) {
    ...
    selectAddressShow() {
      ...
      return this.selectAddress
        ? this.selectAddress.name + "," + this.selectAddress.tel
        : "您还没有设置收货地址";
    },

    selectPro() {
      // 计算需要购买的商品数据 需要给确认订单组件传过去
  ...
  methods: {
    ...
    handleCheckAll(value) {
      //通过判断所有商品是否都被选中来判断全选框的功能式全选还是反选
      if (this.checked.length === this.cartNum) {
        //如果所有商品都被选中，那么就是反选
